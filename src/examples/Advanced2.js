...
function Advanced2() {
  ...
  const queriesData = ()=>{
    Axios.get('https://www.incraftiv.com/downloads/pixstory-api.json')
    .then(res=>res.json)
    .then((data)=>{
      setQueries(data)
    });
  ...
  return (
    <>
          ...
          <div className="cardContainer">
            {stickyNotes.map((character, index) => (
              <TinderCard
                ref={childRefs[index]}
                className={`swipe ${
                  index === stickyNotes.length - 1 ? "active" : ""
                }`}
                key={character}
                  ...
                  <div className="card-details">
                    ...
                    <div className="card-logo">
                      <h4>
                        <img
                          src={
                            process.env.PUBLIC_URL + "/img/main-page-icon.svg"
                          }
                          alt="Icon"
                        />
      ...
      >
        <div className={`fixed-bottom message-section ${messageBox}`}>
